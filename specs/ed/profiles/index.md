...
The [Optional Modules](/ed#optional-modules) published in the Editor's Draft are the official
optional capability family. A profile can require one or more optional modules when a capability
needs shared semantics across tools. Opaque Core `extensions` remain outside semantic profile claims
except for preservation and pass-through behavior.
...
This draft does not define a JSON-LD property for profile declarations. Documents MAY contain
deployment-private profile metadata in `extensions`, but such metadata is not a standardized UJG
profile claim and cannot affect profile conformance.
...
The `#core` profile requires [UJG Core](/ed/core). It is suitable for tools that exchange UJG document
...
The `#graph` profile requires [UJG Core](/ed/core) and [UJG Graph](/ed/graph). It is suitable for tools that exchange
...
The `#surface-runtime` profile requires [UJG Core](/ed/core), [UJG Graph](/ed/graph), [UJG Surface](/ed/surface),
and [UJG Runtime](/ed/runtime). It is suitable for tools that connect runtime observations to
graph-facing surfaces.

### Mapping Metrics {data-cop-concept="mapping-metrics"}

The `#mapping-metrics` profile requires [UJG Core](/ed/core), [UJG Graph](/ed/graph), [UJG Surface](/ed/surface),
[UJG Runtime](/ed/runtime), [UJG Mapping](/ed/mapping), and [UJG Metrics](/ed/metrics). It is suitable for
tools that compute mapping-derived measurements over observed journeys.

### Localized Observability {data-cop-concept="localized-observability"}

The `#localized-observability` profile requires [UJG Core](/ed/core), [UJG Graph](/ed/graph),
[UJG Surface](/ed/surface), [UJG Localization](/ed/modules/l10n), and
[UJG Observability](/ed/modules/observability). It is suitable for tools that recognize surface
instances through accessibility-oriented locators and localized names or descriptions.

### Design System {data-cop-concept="design-system"}

The `#design-system` profile requires [UJG Core](/ed/core), [UJG Graph](/ed/graph),
[UJG Surface](/ed/surface), and [UJG Design System](/ed/modules/design-system). It is suitable for
tools that connect graph-facing surfaces to design-system components, templates, slots, token
sources, and realizations.

### Automation Resources {data-cop-concept="automation-resources"}

The `#automation-resources` profile requires [UJG Core](/ed/core), [UJG Graph](/ed/graph),
[UJG Surface](/ed/surface), [UJG Phase](/ed/modules/phase),
[UJG Observability](/ed/modules/observability), [UJG Entry Binding](/ed/modules/entry-binding),
[UJG Effect](/ed/modules/effect), and [UJG Artifact](/ed/modules/artifact), including the dependency
closure of those modules. It is suitable for automation tools that need stable entry selection
values, observable surface/resource context, phase-oriented grouping, declared effects, and portable
resources.
